...
import com.tecent.student_assessment.extraFunctions.ExtraFunctions.getFullBranch
import com.tecent.student_assessment.extraFunctions.ExtraFunctions.getFullSemester
import com.tecent.student_assessment.extraFunctions.ExtraFunctions.getFullUniversity
import com.tecent.student_assessment.extraFunctions.ExtraFunctions.getSmallBranch
import com.tecent.student_assessment.extraFunctions.ExtraFunctions.getSmallSemester
import com.tecent.student_assessment.extraFunctions.ExtraFunctions.getSmallUniversity
      ...
      object : VolleyMultipartRequest(Method.POST, url, Response.Listener { response ->
        ...
        try {
          ...
          if (status == "successful") {
            dialog.dismiss()
            Toast.makeText(this@Profile, "DP Changed", Toast.LENGTH_SHORT)
                .show()
            requestQueue.add(
                ExtraFunctions.createImageRequestFromUrl(
                    ExtraFunctions.serverurl + "userdp/" + userdp, iv_profile_image
          ...
          if (status == "error") {
            dialog.dismiss()
            Toast.makeText(this@Profile, "Error! Please try again later...", Toast.LENGTH_SHORT)
                .show()
          }
        ...
        dialog.dismiss()
        Toast.makeText(this@Profile, "Volley Error", Toast.LENGTH_SHORT)
            .show()
      }) {
        override fun getParams(): Map<String, String> {
        ...
        override fun getByteData(): Map<String, DataPart>? {
          ...
          params["profileimage"] = DataPart(
              "profile_image.jpg",
              AppHelper.getFileDataFromDrawable(baseContext, iv_profile_image.getDrawable()),
              "image/jpeg"
          )
