    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        send_message.setOnClickListener {
            if (et_discussionText.text.trim().toString().length<1)
                Toast.makeText(this@DiscussionForum, "Please write something", Toast.LENGTH_SHORT).show()
            else{
                dialog.show()
...
//                progressBar.setVisibility(View.GONE)
        ...
        et_discussionText.addTextChangedListener(object : TextWatcher {
            ...
            override fun onTextChanged(charSequence: CharSequence, i: Int, i1: Int, i2: Int) {
                if (et_discussionText.text.toString().trim { it <= ' ' }.length >= 1) {
                    ...
                    send_message.setColorFilter(resources.getColor(R.color.colorPrimary))
                } else if (et_discussionText.text.toString().trim { it <= ' ' }.length < 1) {
                    send_message.setColorFilter(resources.getColor(R.color.lightenblue))
                }
            }
    ...
    fun volleyTestDiscussionForum() {
        ...
        val stringRequest = object : StringRequest(Request.Method.POST, url, Response.Listener { response -> jsonParser(response) }, Response.ErrorListener { error ->
            dialog.dismiss()
            Toast.makeText(this@DiscussionForum, error.toString(), Toast.LENGTH_SHORT).show()
    ...
    fun jsonParser(jsontext: String) {
        try {
            ...
            if (result == "successful") {
                ...
                dialog.dismiss()
                Toast.makeText(this@DiscussionForum, "Message uploaded successfully", Toast.LENGTH_SHORT).show()
                volleyDiscussionForumDataRequest()
            }
            if (result == "error") {
                dialog.dismiss()
                Toast.makeText(this@DiscussionForum, "Error! Please try again later...", Toast.LENGTH_SHORT).show()
            }
