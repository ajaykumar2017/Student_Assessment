...
class MyRecyclerDoubtsPostsAnswersRepliesAdapter(
  requestQueue: RequestQueue,
  context: AnswersDoubtsPostsDoubtsActivity,
  doubtPostid: String,
  userid: String,
  replyForArrayListObject: ArrayList<ReplyForDoubtsPostsAnswersObject>
) : RecyclerView.Adapter<DoubtsPostsAnswersRepliesHolder>() {
  ...
  override fun onBindViewHolder(
    doubtsPostsAnswersRepliesHolder: DoubtsPostsAnswersRepliesHolder,
    position: Int
  ) {
    ...
    doubtsPostsAnswersRepliesHolder.replyText.setOnLongClickListener {
      ExtraFunctions(mContext).copyTextToClipboard(
          doubtsPostsAnswersRepliesHolder.replyText.text.toString(),
          doubtsPostsAnswersRepliesHolder.replyText.text
      )
    }
      ...
      popup.setOnMenuItemClickListener { menuItem ->
        when (menuItem.itemId) {
          id.delete_reply -> if (DataUtils.isNetworkStatusAvailable(mContext)) {
            val url = DataUtils.serverurl + "deleteAnswersRepliesOfDoubtsPosts.php"
            val stringRequest =
              object : StringRequest(Request.Method.POST, url, Response.Listener { response ->
                try {
                  ...
                  if (result == "successful") {
                    Toast.makeText(mContext, "Reply Deleted successfully", Toast.LENGTH_SHORT)
                        .show()
  ...
  override fun onCreateViewHolder(
    p0: ViewGroup,
    p1: Int
  ): DoubtsPostsAnswersRepliesHolder {
