...
import kotlinx.android.synthetic.main.activity_create_post_home.et_text_post
import kotlinx.android.synthetic.main.activity_create_post_home.image_path
...
class CreatePostHomeActivity : AppCompatActivity() {
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    btn_post.setOnClickListener {
      postText = et_text_post.text.toString()
          .replace("'", "\\'")
      ...
      try {
        if (requestCode == READ_REQUEST_CODE && resultCode == Activity.RESULT_OK) {
          ...
          if (file.length() < 10 * 1024 * 1024) {
            ...
            if (temp!!.endsWith(".pdf") || temp.endsWith(".jpg") || temp.endsWith(
                    ".png"
                ) || temp.endsWith(".jpeg") ||
                temp.endsWith(".PDF") || temp.endsWith(".JPG") || temp.endsWith(
                    ".PNG"
                ) || temp.endsWith(".JPEG")
            ) {
...
//                                filefullpath = createthumbnailbig(path, filename)
              ...
              Toast.makeText(this, "Oops...unable to read file", Toast.LENGTH_LONG)
                  .show()
            }
          } else {
            val snackbar = Snackbar.make(
    ...
    try {
      ...
      if (result == "successful") {
        ...
        sharedPreferencesEditPosts.putString(
            "posts", (Integer.parseInt(sharedPreferences.getString("posts", "")!!) + 1).toString()
        )
