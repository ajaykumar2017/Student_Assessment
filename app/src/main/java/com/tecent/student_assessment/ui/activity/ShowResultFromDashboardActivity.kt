...
class ShowResultFromDashboardActivity : AppCompatActivity() {
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    setContentView(
        layout.activity_show_result_from_dashboard
    )
    ...
    var pieChart = findViewById<PieChart>(
        id.pie_chart
    )
    var barChart = findViewById<BarChart>(
        id.barchart
    )
...
//        val bardataset = BarDataSet(NoOfEmp, "No Of Employee")
  ...
  fun getDate(cld: Calendar): String {
    var curDate = (cld.get(Calendar.YEAR)
        .toString() + "/" + (cld.get(Calendar.MONTH) + 1) + "/"
