...
class CreatePostQueryDoubtsActivity : AppCompatActivity() {
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    tv_btn_post.setOnClickListener {
      postText = et_post_text.text.toString()
          .replace("'", "\\'")
      if (postText.trim().length < 5) {
        ...
        if (ExtraFunctions.isNetworkStatusAvailable(this)) {
          ...
          dialog.dismiss()
          Toast.makeText(
              this@CreatePostQueryDoubtsActivity, "No internet connection!", Toast.LENGTH_SHORT
          )
              .show()
        }
      } else {
    ...
    et_post_text.addTextChangedListener(object : TextWatcher {
      ...
      override fun onTextChanged(
        ...
        if (et_post_text.text.toString()
                .trim { it <= ' ' }.length >= 5
        ) {
          tv_btn_post.setBackgroundResource(
              color.green
          )
        } else if (et_post_text.text.toString()
                .trim { it <= ' ' }.length < 5
        ) {
          tv_btn_post.setBackgroundResource(
              color.smalldarkgrey
      ...
      try {
        if (requestCode == READ_REQUEST_CODE && resultCode == Activity.RESULT_OK) {
          ...
          if (file.length() < 10 * 1024 * 1024) {
            ...
            if (temp!!.endsWith(".jpg") || temp.endsWith(".png") || temp.endsWith(
                    ".jpeg"
                ) || temp.endsWith(".JPG") || temp.endsWith(".PNG") || temp.endsWith(".JPEG")
            ) {
    ...
    try {
      ...
      if (result == "successful") {
        ...
        sharedPreferencesEditPosts.putString(
            "doubts", (Integer.parseInt(sharedPreferences.getString("doubts", "")!!) + 1).toString()
        )
